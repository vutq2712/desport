...
import { getUserNotInTeamByUserName } from "@app/api/user-team/my-teams";
import { addTeamMember } from "@app/api/team/add-member";
import { confirm } from '@app/dekits/modal'

export function MyTeamFullMembers(props: any) {
  const [isEnableAdd, setIsEnableAdd] = useState(false);
  const fetchOptions = useCallback((keyword: string) => {
    return getUserNotInTeamByUserName({
      key_search: keyword,
  ...
  const handleUpdate = useCallback((values) => {
    confirm({ content: `Are you sure you want to add this user?` }).subscribe(answer => {
      if (!answer) return;

      addTeamMember({team_id: props.team_id, member: values.selectedUser}).subscribe(res => {
        if(res.code == 200)
          props.onrefresh();
      })
    })
  ...
  return (
    ...
    >
      <div className={`de-card ${props.readOnly ? 'de-pb-12px' : ''}`}>
        <div className='de-card-header de-mb-3 align-items-center'>
          <div className='de-card-title'>Team Members</div>
          {
            props.readOnly ?
              <></>
              ...
              // </button>
              :
              <div className='input-group align-items-center justify-content-end' style={{ maxWidth: '350px' }}>
                <div className='de-form-control-with-icon'>
                  ...
                  <Select placeholder="Search by username..." onChange={() => {setIsEnableAdd(true)}} name='selectedUser' fetchOption={fetchOptions} hasSearch />
                </div>
                <div className='input-group-append de-ms-2'>
                  <button disabled={!isEnableAdd} type='submit' className='de-btn de-btn-outline-secondary de-px-2'>
                    <span>Add</span>
                  </button>
                </div>
              </div>
        ...
        <div className='de-card-body'>
          <div className='row'>
            {props.members && props.members.map(member => 
              <div key = {member._id} className='col-lg-3 col-md-6'>
                <TeamMemberCard onrefresh={props.onrefresh}  team_id= {props.team_id} memberInfo = {member} readOnly={props.readOnly} isCaptain={props.captain == member._id} />
              </div>)                 
            }
          </div>
          {
            props.isListingPage ?
              <button onClick={props.onViewDetail} type='button' className='de-btn de-btn-sm de-btn-secondary'>
