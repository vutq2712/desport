...
import { MemberProfileSelectCard } from '../member-profile-select-card';

function SelectMemberProfileModal(props: any) {
  const { modalRef, data } = props;
  const [selectedIndex, setSelectedIndex] = useState(-1);
  const [profiles, changeProfiles] = useState<Array<any>>(data?.profileInfo || []);

  useEffect(() => {
    changeProfiles(data?.profileInfo || []);
  }, [data])

  const onFilter = (e) => {
    if (!data || !data.profileInfo?.length || !e.target) return;

    const newProfiles = data.profileInfo.filter(item => (item.profile_name as string).toLowerCase().indexOf(e.target.value.toLowerCase()) > -1);
  ...
  return (
    <div className='modal-body de-p-3'>
      <div className='modal-sub-title de-mb-3' id='exampleModalLabel'>Choose profile name</div>
      <div className='modal-inner'>
        ...
        <div className='de-member-profiles'>
          {profiles.map((profile, index) => (
            <MemberProfileSelectCard
              key={index}
              logo={data._id.avatar || '/assets/images/avatar-2.png'}
      ...
      <div className='modal-actions'>
        <button type='button' disabled={selectedIndex === -1} className='de-btn de-btn-primary w-100' onClick={() => modalRef.close(data.profileInfo[selectedIndex])}>
          <span>Confirm</span>
        </button>
      </div>
    </div>
...
export function SelectMemberProfile({ disabled, member, onSelectedProfile }) {
  const openSelectMemberProfile = useCallback(() => {
    const modal = openModal(SelectMemberProfileModal, { dialogClassName: 'de-modal-md', data: member });

    modal.afterClosed().subscribe(data => {
      onSelectedProfile(data);
    })
  }, [member, onSelectedProfile]);

  return (
    <button disabled={disabled} className='de-btn de-btn-select' type='button' onClick={openSelectMemberProfile}>
