...
import { TeamSelectCard } from '../team-select-card';
import { TeamData } from '@app/api/tournament/register/get-list-team';

interface SelectTeamProps {
  teams: TeamData[],
  teamSelected: TeamData,
  onSelected: (team: TeamData) => void;
}

function SelectTeamModal(props: any) {
  const { modalRef, data } = props;
  const [selectedIndex, setSelectedIndex] = useState(-1);
  const [teams, changeTeams] = useState<TeamData[]>(data?.teams || []);
  ...
  const onFilter = (e) => {
    if (!data || !data.teams?.length || !e.target) return;

    const newTeams = data.teams.filter(item => (item.name as string).toLowerCase().indexOf(e.target.value.toLowerCase()) > -1);
  ...
  return (
    <div className='modal-body de-p-3'>
      <div className='modal-sub-title de-mb-1'>team in</div>
      <div className='modal-title modal-title-2 de-mb-3' id='exampleModalLabel'>{ data?.teamSelected?.name }</div>
      <div className='modal-inner'>
        <div className='de-form-group form-group'>
          <div className='de-form-control-with-icon'>
            ...
            <input className='de-form-control form-control' type='text' onChange={onFilter} placeholder='Search team' />
          </div>
        </div>
        {teams.map((team, index) => (
          <TeamSelectCard
            key={index}
            logo={team.logo || '/assets/images/team-logo-1.png'}
            name={team.name}
      ...
      <div className='modal-actions'>
        ...
        <button
          ...
          className='de-btn de-btn-primary w-100'
          onClick={() => modalRef.close(data.teams[selectedIndex])}
        >
          <span>Confirm</span>
        </button>
      </div>
...
export function SelectTeam({ teams, teamSelected, onSelected }: SelectTeamProps) {
  const openSelectTeam = useCallback(() => {
    if (!teams) return;

    const modal = openModal(SelectTeamModal, { dialogClassName: 'de-modal-md', data: { teams, teamSelected } });

    modal.afterClosed().subscribe(data => {
      onSelected(data);
    })
  }, [teams, teamSelected, onSelected]);

  return (
    <button className='de-btn de-btn-select' type='button' onClick={openSelectTeam}>
      <span>Team name</span>
      <svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
